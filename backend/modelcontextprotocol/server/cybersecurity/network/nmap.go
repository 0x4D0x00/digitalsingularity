...
// NmapDataService 处理Nmap扫描相关的数据服务
// 参考 backend/silicoid/database/service.go 的设计模式
type NmapDataService struct {
	dataService *datahandle.CommonReadWriteService
	dbName      string
}

// NewNmapDataService 创建Nmap数据服务实例
func NewNmapDataService(dataService *datahandle.CommonReadWriteService) *NmapDataService {
	return &NmapDataService{
		dataService: dataService,
		dbName:      "storagebox", // Nmap使用storagebox数据库
	}
}

// GetDatabaseName 返回数据库名
func (s *NmapDataService) GetDatabaseName() string {
	return s.dbName
}

// 包级别的默认服务实例
var defaultNmapService *NmapDataService

// init 初始化默认的服务实例
func init() {
	dataService, err := datahandle.NewCommonReadWriteService("storagebox")
	if err != nil {
		log.Printf("Failed to create default nmap data service: %v", err)
		return
	}
	defaultNmapService = NewNmapDataService(dataService)
}

// 全局数据服务实例，避免重复创建
var (
	storageboxDataService *datahandle.CommonReadWriteService
	storageboxServiceOnce sync.Once
)
...
func getStorageboxDBConfig() DatabaseConfig {
	...
	if defaultNmapService == nil || defaultNmapService.dataService == nil {
		log.Printf("Nmap data service not available")
	...
	// 通过服务实例获取配置，简洁明了
	config := defaultNmapService.dataService.GetDbConfig()
	return DatabaseConfig{
